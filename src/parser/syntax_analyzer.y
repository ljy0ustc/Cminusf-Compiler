...
/* 当栈类型是一个联合体的时候, 你必须使用%token或者其它记号声明来指明记号的语义值类型. 语义值类型由中括号分隔. */
%token <node> ERROR 
%token <node> ADD
%token <node> SUB
%token <node> MUL
...
/* 如果你要指明结合性和优先级, 你可以使用%left,%right或者%nonassoc 代替%token. 参阅 操作符优先级-Operator Precedence. 
声明在后的优先级更高*/
...
/* Example:
program: declaration-list {$$ = node( "program", 1, $1);}
       ;
*/
